...
const CharacterCreationChart = () => {
  ...
  useEffect(() => {
    ...
    const filteredCharacterData = characterData.filter((character) => {
      ...
      return createdDate.getFullYear() === 2017; // Filter by year (2017)
    });
    ...
    const chartOptions = {
      ...
      scales: {
        ...
        y: {
          type: 'time',
          time: {
            unit: 'hour', // Display time up to hours
            displayFormats: {
              hour: 'MMM DD, YYYY HH:mm' // Format to display month, day, year, and time
            }
          },
          title: {
            display: true,
          ...
          ticks: {
            stepSize: 1 // Customize the step size for y-axis ticks (1 hour)
          }
        }
      }
    };
