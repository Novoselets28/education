...
const MainPage = () => {
  ...
  const commitChanges = ({ deleted }) => {
    if (deleted) {
      const deletedRows = deleted.map(deletedRowIndex => rows[deletedRowIndex].id); // Assuming each row has a unique ID
  
      // Filter out the deleted rows from the 'rows' state
      const newRows = rows.filter(row => !deletedRows.includes(row.id));
  
      // Log to check the deleted rows and the newRows
