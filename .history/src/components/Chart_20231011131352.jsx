...
const CharacterCreationChart = () => {
  ...
  useEffect(() => {
    ...
    const chartData = {
      labels: filteredCharacterData.map((character) => character.created), // Swap X and Y
      datasets: [
        {
          label: 'Characters',
          ...
          borderWidth: 2,
          fill: false,
          data: filteredCharacterData.map((character, index) => ({ x: character.name, y: index })) // Swap X and Y
        }
      ]
    };
