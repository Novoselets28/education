...
const CharacterCreationChart = () => {
  ...
  useEffect(() => {
    ...
    // Find the minimum and maximum creation dates in the filtered data
    const minDate = new Date(Math.min(...characterData2017.map((character) => new Date(character.created).getTime())));
    const maxDate = new Date(Math.max(...characterData2017.map((character) => new Date(character.created).getTime())));
