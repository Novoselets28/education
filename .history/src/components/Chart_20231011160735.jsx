...
const CharacterCreationChart = () => {
  ...
  useEffect(() => {
    ...
    // Create an array of timestamps within the year 2017
    const timestamps2017 = [];
    for (let i = 0; i < 24; i++) {
      timestamps2017.push(new Date(2017, 10, 1, i).getTime()); // November is month 10 (0-indexed)
    }
    ...
    const chartOptions = {
      ...
      scales: {
        ...
        y: {
          ...
          time: {
            unit: 'hour',
            stepSize: 1, // 1 hour step
            displayFormats: {
              hour: 'MMM DD, YYYY HH:mm' // Format to display day, month, year, and time
            },
            title: {
              display: true,
          ...
          ticks: {
            max: new Date(2017, 10, 1, 23).getTime(), // Set the maximum date within the year 2017
            min: new Date(2017, 10, 1, 0).getTime() // Set the minimum date within the year 2017
          }
        }
      }
    };
