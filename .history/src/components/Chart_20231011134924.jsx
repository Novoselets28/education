...
const CharacterCreationChart = () => {
  ...
  useEffect(() => {
    ...
    const chartData = {
      labels: filteredCharacterData.map((character) => {
        const createdDate = new Date(character.created);
        return createdDate.toLocaleString(); // Display the exact time
      }),
      datasets: [
        {
          label: 'Created Date',
          ...
          borderWidth: 2,
          fill: false,
          data: filteredCharacterData.map((character) => new Date(character.created).getTime())
        }
      ]
    };
    ...
    const chartOptions = {
      ...
      scales: {
        x: {
          type: 'time',
          time: {
            parser: 'MMM DD, YYYY, hh:mm A', // Format for displaying time
            unit: 'day',
            displayFormats: {
              day: 'MMM DD, YYYY'
            },
            title: {
              display: true,
              text: 'Characters Created in November'
            }
          }
        },
        y: {
  ...
  return (
    <div>
      <div style={chartSize}>
        {lineChartData && lineChartOptions && <Line data={lineChartData} options={lineChartOptions} />}
      </div>
      <div style={chartSize}>
