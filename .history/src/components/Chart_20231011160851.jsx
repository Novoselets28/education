...
const CharacterCreationChart = () => {
  ...
  useEffect(() => {
    ...
    // Filter characters created in November 2017
    const characters2017 = characterData.filter((character) => {
      const createdDate = new Date(character.created);
      return createdDate.getFullYear() === 2017 && createdDate.getMonth() === 10; // November (0-indexed)
    });

    // Extract timestamps from characters created in November 2017
    const timestamps2017 = characters2017.map((character) => new Date(character.created).getTime());
